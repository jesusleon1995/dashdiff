...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Dashdiff address (e.g. %1)").arg("XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg"));
#endif
...
{
    // return if URI is not valid or is no dashdiff: URI
    if(!uri.isValid() || uri.scheme() != QString("dashdiff"))
        return false;
...
{
    // Convert dashdiff:// to dashdiff:
    //
    //    Cannot handle this later, because dashdiff:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dashdiff://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "dashdiff:");
    }
...
{
    QString ret = QString("dashdiff:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open dashdiff.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dashdiff Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dashdiff Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Dashdiff Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Dashdiff Core*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "dashdiffcore.desktop";
    return GetAutostartDir() / strprintf("dashdiffcore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a dashdiffcore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Dashdiff Core\n";
        else
            optionFile << strprintf("Name=Dashdiff Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Dashdiff Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Dashdiff Core app to startup item list
