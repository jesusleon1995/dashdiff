...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Dashdiff related functionality
...
/*
    ...
    allow endless transaction that would bloat Dashdiff and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Dashdiff specific functionality
    ...
    RenameThread("dashdiff-ps-server");
