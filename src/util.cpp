...
const char * const BITCOIN_CONF_FILENAME = "dashdiff.conf";
const char * const BITCOIN_PID_FILENAME = "dashdiffd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dashdiff" is a composite category enabling all Dashdiff-related debug output
            if(ptrCategory->count(string("dashdiff"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DashdiffCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DashdiffCore
    // Mac: ~/Library/Application Support/DashdiffCore
    // Unix: ~/.dashdiffcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DashdiffCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DashdiffCore";
#else
    // Unix
    return pathRet / ".dashdiffcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dashdiff.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dashdiff.conf
