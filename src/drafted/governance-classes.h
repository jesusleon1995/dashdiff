 ...
 *
 *   // network
 *   CDashdiffNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.dashdiff.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CDashdiffNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CDashdiffNetwork(UniValue objIn)
    {
...
// // can be under: DashdiffNetwork
// //   -- signature requirements : Key1(User)
// class CDashdiffNetworkVariable : public CGovernanceObject
// {
// private:
